...
The project README is the first surface contributors see. It should identify the media aggregator and its current preview clearly near the top, explain how vendored ASDLC references relate to repo-specific docs, point contributors at the current runtime and verification commands, and keep screenshot refresh manual.
...
- **Update model:** manual refresh when the product UI changes materially
...
- Do not let the committed screenshot drift far from the current product UI.
...
- Do not let the README drift away from the actual commands, ports, or source layout used by the current project.
...
- [ ] The README identifies the media aggregator as a Cloudflare Worker served with Wrangler near the top.
...
- `README.md` should continue to describe the current product source layout and verification flow accurately.
...
- The screenshot should continue to represent the current product surface closely enough to be useful.
- Screenshot support must remain manual unless a later ADR changes that rule.

### Verification

- **Screenshot refresh:** manual developer update when the product UI changes materially
...
- Then: they can tell quickly that the media aggregator is a Cloudflare Worker served with Wrangler and centered on server-rendered HTML
...
**Scenario: Product UI changes materially**
